...
fn main() {
    ...
    let exclusive: ExclusiveMode<'_> = shared.as_exclusive();
    // Create a new cell with the initial value of `1`. This cell can be
    // accessed `mut`-ably using an `ExclusiveMode`.
    let mut exclusive_cell: ExclusiveCell<usize> = exclusive.new_cell(1);
    // Obtain a SharedCell, which can be converted to an `&T` using a
    // `SharedMode`.
    let shared_cell: SharedCell<usize> = exclusive_cell.as_shared();
    ...
    let mut cell_contents: RefMut<'_, usize> = exclusive_cell.get_mut(exclusive);
    *cell_contents = 2;
